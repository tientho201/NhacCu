...
public class Admin_DsTaiKhoan extends JPanel {
	...
	public void initComponents() {
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"M\u00E3 Ng\u01B0\u1EDDi D\u00F9ng", "H\u1ECD v\u00E0 T\u00EAn", "Gi\u1EDBi T\u00EDnh", "Ng\u00E0y Sinh", "S\u1ED1 \u0110i\u1EC7n Tho\u1EA1i", "\u0110\u1ECBa Ch\u1EC9", "Vai Tr\u00F2", "Ghi Ch\u00FA", "Tr\u1EA1ng Th\u00E1i"
			}
		));
...
//		 Đổ dữ liệu vào DefaultTableModel
		for (NhanVienDTO nv : listNV) {
			if (nv.getEnable() == 1) {
				model.addRow(new Object[] { nv.getMaNhanVien() ,nv.getTenDangNhap(), nv.getTenNhanVien() , nv.getGioiTinh() , nv.getNgaySinh() , nv.getSDT() , nv.getDiaChi() , nv.getChucVu() , nv.getGhiChu() , "Hoạt Động" });				
			}else {
				model.addRow(new Object[] { nv.getMaNhanVien() ,nv.getTenDangNhap(), nv.getTenNhanVien() , nv.getGioiTinh() , nv.getNgaySinh() , nv.getSDT() , nv.getDiaChi() , nv.getChucVu() , nv.getGhiChu() , "Tạm Dừng" });				
			}

		}
		for (UserDTO us : listUS) {
			if (us.getEnable() == 1) {
				model.addRow(new Object[] { us.getMaUser() ,us.getTenDangNhap(), us.getHovaTen() ,us.getGioiTinh() , us.getNgaySinh() , us.getSDT() , us.getDiaChi() , "KhachHang" , us.getGhiChu() , "Hoạt Động" });				
			}else {
				model.addRow(new Object[] { us.getMaUser() ,us.getTenDangNhap(), us.getHovaTen() ,us.getGioiTinh() , us.getNgaySinh() , us.getSDT() , us.getDiaChi() , "KhachHang" , us.getGhiChu() , "Tạm Dừng" });				
			}
...
//		// Cho phép table sắp xếp
	...
	public boolean kiemtraRong() {
		if (textFieldMaNguoiDung.getText() == null || textFieldMaNguoiDung.getText().equals("")) {
		    JOptionPane.showMessageDialog(null, "Mã Người Dùng không được trống!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			textFieldMaNguoiDung.requestFocus();
			return false;
		}
		if (textFieldDiaChi.getText() == null || textFieldDiaChi.getText().equals("")) {
			 JOptionPane.showMessageDialog(null, "Địa Chỉ không được trống!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			 textFieldDiaChi.requestFocus();
			return false;
		}
		if (textFieldHovaTen.getText() == null || textFieldHovaTen.getText().equals("")) {
			 JOptionPane.showMessageDialog(null, "Họ và Tên không được trống!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			 textFieldHovaTen.requestFocus();
			return false;
		}
		if (textFieldNgaySinh.getText() == null || textFieldNgaySinh.getText().equals("")) {
			 JOptionPane.showMessageDialog(null, "Ngày Sinh không được trống!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			 textFieldNgaySinh.requestFocus();
			return false;
		}
		if (textFieldSoDienThoai.getText() == null || textFieldSoDienThoai.getText().equals("")) {
			 JOptionPane.showMessageDialog(null, "Số Điện Thoại không được trống!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			 textFieldSoDienThoai.requestFocus();
			return false;
		}
		if (textFieldTenDangNhap.getText() == null || textFieldTenDangNhap.getText().equals("")) {
			 JOptionPane.showMessageDialog(null, "Tên Đăng Nhập không được trống!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
	...
	public void them() {
		if(kiemtraRong() ) {
			if ( !nvBUS.checkMaNV(textFieldMaNguoiDung.getText()) && !usBUS.checkMaUser(textFieldMaNguoiDung.getText())){
				if (!accBUS.checkTenDangNhap(textFieldTenDangNhap.getText())) {
					if (comboBoxVaiTro.getSelectedItem().equals("KhachHang")) {
						if (comboBoxTrangThai.getSelectedItem().equals("Hoạt Động")) {
							AccountDTO accDTO = new AccountDTO(textFieldTenDangNhap.getText() , textFieldTenDangNhap.getText() , "KhachHang" , 1 );
							accBUS.addAcc(accDTO);
							UserDTO usDTO = new UserDTO(textFieldMaNguoiDung.getText() , textFieldTenDangNhap.getText() , textFieldHovaTen.getText() , comboBoxGioiTinh.getSelectedItem().toString() , textFieldNgaySinh.getText() , textFieldSoDienThoai.getText() , textFieldDiaChi.getText() , textAreaMoTa.getText() , 1);
							usBUS.addUs(usDTO);						
						}else {
							AccountDTO accDTO = new AccountDTO(textFieldTenDangNhap.getText() , textFieldTenDangNhap.getText() , "KhachHang" , 0 );
							accBUS.addAcc(accDTO);
							UserDTO usDTO = new UserDTO(textFieldMaNguoiDung.getText() , textFieldTenDangNhap.getText() , textFieldHovaTen.getText() , comboBoxGioiTinh.getSelectedItem().toString() , textFieldNgaySinh.getText() , textFieldSoDienThoai.getText() , textFieldDiaChi.getText() , textAreaMoTa.getText() , 0);
							usBUS.addUs(usDTO);		
						}
					}else {
						if (comboBoxTrangThai.getSelectedItem().equals("Hoạt Động")) {
							AccountDTO accDTO = new AccountDTO(textFieldTenDangNhap.getText() , textFieldTenDangNhap.getText() , comboBoxVaiTro.getSelectedItem().toString() , 1 );
							accBUS.addAcc(accDTO);
							NhanVienDTO nvDTO = new NhanVienDTO(textFieldMaNguoiDung.getText() , textFieldTenDangNhap.getText() , textFieldHovaTen.getText() , comboBoxGioiTinh.getSelectedItem().toString() , textFieldNgaySinh.getText() , textFieldSoDienThoai.getText() , textFieldDiaChi.getText() ,comboBoxVaiTro.getSelectedItem().toString(), textAreaMoTa.getText() , 1);						
							nvBUS.addNV(nvDTO);
						}else {
							AccountDTO accDTO = new AccountDTO(textFieldTenDangNhap.getText() , textFieldTenDangNhap.getText() , comboBoxVaiTro.getSelectedItem().toString() , 0 );
							accBUS.addAcc(accDTO);
							NhanVienDTO nvDTO = new NhanVienDTO(textFieldMaNguoiDung.getText() , textFieldTenDangNhap.getText() , textFieldHovaTen.getText() , comboBoxGioiTinh.getSelectedItem().toString() , textFieldNgaySinh.getText() , textFieldSoDienThoai.getText() , textFieldDiaChi.getText() ,comboBoxVaiTro.getSelectedItem().toString(), textAreaMoTa.getText() , 0);						
							nvBUS.addNV(nvDTO);
						}
					}
